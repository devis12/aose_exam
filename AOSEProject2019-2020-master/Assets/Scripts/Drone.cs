...
public class Drone : Agent
{
    ...
    {   
        return  !anim.GetBool("isActive") && toCharging;//not active(not flying) && last trip to charging station
    }

    public IEnumerator Goto(GameObject go)
    {   
        toCharging = (go == chargingStation);//directed towards charging station
